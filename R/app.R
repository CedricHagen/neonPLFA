...
DRIVER_VARS <- c(
  ...
  "pH (in water)" = "soilInWaterpH",
  "pH (in CaCl2)" = "soilInCaClpH",
  ...
  "F:B ratio" = "FB_ratio",
  "stress index (cy19:0/18:1ω7c)" = "stress_index_cy19"
)
  ...
  sidebarLayout(
    sidebarPanel(
      ...
      passwordInput(
        ...
        value = "",
        placeholder = "Paste your NEON API token…"
      ),
      tags$p(
        style = "font-size: 0.85em; color: #666; margin-top: -8px;",
        "A NEON API token is required to download data. You can get one for free by creating an account on the ",
        tags$a(href = "https://data.neonscience.org/",
               target = "_blank", rel = "noopener", "NEON data portal"),
        ", then generating a token under your ",
        tags$a(href = "https://data.neonscience.org/myaccount",
               target = "_blank", rel = "noopener", "account settings"),
        " (see the ",
        tags$a(href = "https://www.neonscience.org/resources/learning-hub/tutorials/neon-api-tokens-tutorial",
               target = "_blank", rel = "noopener", "token tutorial"),
        "). Your token is used only for this session and is never stored or shared."
      ),
      ...
      tabsetPanel(
        tabPanel("Read me",
                 tags$div(
                   ...
                   tags$h3("NEON PLFA Explorer"),
                   tags$p("This app downloads NEON phospholipid fatty acid (PLFA) data and paired soil ",
                          "properties, computes a set of soil microbial-health indicators, and lets you ",
                          "explore them across sites, time, seasons, and environmental drivers."),

                   tags$h4("Data sources"),
                   tags$ul(
                     tags$li(tags$b("DP1.10104.001"), " — Soil microbe biomass (PLFA)."),
                     tags$li(tags$b("DP1.10086.001"), " — Soil physical and chemical properties (periodic): temperature, moisture, C/N, isotopes, pH."),
                     tags$li(tags$b("DP1.00096.001"), " — Soil physical and chemical properties (Megapit): soil taxonomy / soil type.")
                   ),

                   tags$h4("How to use"),
                   tags$ol(
                     tags$li("Paste your NEON API token (required — see the sidebar for how to get one for free)."),
                     tags$li("Choose a NEON data release."),
                     tags$li("Select one or more PLFA sites and a collection-date range."),
                     tags$li("Click ", tags$b("Run analysis"), " to download and process the data."),
                     tags$li("Pick a ", tags$b("Display site"), " to populate the plots and tables."),
                     tags$li("Explore the tabs, then optionally download the processed dataset (CSV) or the plots (ZIP).")
                   ),

                   tags$h4("Indicator definitions"),
                   tags$ul(
                     tags$li(tags$b("Microbial biomass"), " (nmol PLFA / g soil) — total lipid concentration with the c18:0 contamination correction applied (post-2021 NEON guidance), floored at zero."),
                     tags$li(tags$b("Fungal PLFA"), " — 18:2ω6,9 marker concentration."),
                     tags$li(tags$b("Bacterial PLFA"), " — sum of common bacterial markers (branched i/a 15:0–17:0, 10-methyl 16:0–18:0, monoenoic ω7/ω9, and cyclopropyl cy17:0 / cy19:0)."),
                     tags$li(tags$b("F:B ratio"), " — fungal PLFA ÷ bacterial PLFA."),
                     tags$li(tags$b("Stress index (cy19:0 / 18:1ω7c)"), " — cyclopropyl:precursor physiological-stress ratio. Cyclopropyl fatty acids form from their monoenoic precursors under stress (stationary phase, low pH, nutrient limitation), so higher values indicate greater stress."),
                     tags$li(tags$b("Biomass per g C"), " — microbial biomass normalized to soil organic carbon (nmol PLFA / g C)."),
                     tags$li(tags$b("C:N ratio"), " — soil organic C% ÷ total N%.")
                   ),
                   tags$p(tags$em("Horizons: "), "“O” (organic) and “M” (mineral) are inferred from the NEON sampleID and soil-core metadata."),

                   tags$h4("Citation"),
                   tags$p("If you use this app or its outputs, please cite the accompanying paper: ",
                          tags$em("(citation to be added upon publication)"), "."),
                   tags$p("Data provided by the National Ecological Observatory Network (NEON), a program ",
                          "sponsored by the U.S. National Science Foundation and operated under cooperative ",
        ...
        tabPanel("Trends",
                 tags$p("These coefficients are from linear regression of each metric against time (year); ",
                        tags$em("slope_per_year"), " is the estimated change in the metric per year. ",
                        "Estimates require ≥ 3 unique years — empty results usually mean insufficient temporal coverage."),
...
server <- function(input, output, session) {
  ...
  observeEvent(input$run, {
    ...
    if (is.null(input$neon_token) || !nzchar(trimws(input$neon_token))) {
      rv$status <- "ERROR: A NEON API token is required. Paste your token above (get one free from your NEON account)."
      output$status_text <- renderText(rv$status)
    ...
    withProgress(message = "Running NEON PLFA Explorer…", value = 0, {
  ...
  output$summary_table <- renderDT({
    ...
    summ <- d %>%
      summarize(
        ...
        stress_median = median(stress_index_cy19, na.rm = TRUE),
        organicC_median = median(organicCPercent, na.rm = TRUE),
        N_median = median(nitrogenPercent, na.rm = TRUE)
      )

    num_cols <- intersect(
      c("biomass_median", "FB_median", "stress_median", "organicC_median", "N_median"),
      names(summ)
    )
  ...
  ts_plot <- function(df, y, ylab) {
    ggplot(df, aes(x = collectDate, y = .data[[y]])) +
      geom_point(size = 1.8, alpha = 0.7, na.rm = TRUE) +
  ...
  output$ts_stress <- renderPlot({
    req(display_df())
    ts_plot(display_df(), "stress_index_cy19", "Stress index (cy19:0 / 18:1ω7c)")
  })
  ...
  output$trends_table <- renderDT({
    ...
    dt <- datatable(tr, options = list(pageLength = 25), rownames = FALSE)
    round_cols <- intersect(c("slope_per_year", "r2"), names(tr))
    if (nrow(tr) > 0 && length(round_cols) > 0) dt <- dt %>% formatRound(columns = round_cols, digits = 4)
    if (nrow(tr) > 0 && "p_value" %in% names(tr)) dt <- dt %>% formatSignif(columns = "p_value", digits = 3)
    dt
  })
  ...
  build_drivers_outputs <- reactive({
    ...
    if (hz_sel == "Both") {
      ...
      # Overall fit summary, shown as a subtitle so it never overlaps the points
      ...
      p <- ggplot(df, aes(x = x, y = y, color = horizonClass)) +
        ...
        labs(x = xlab, y = ylab, color = "Horizon", subtitle = sub_txt) +
        theme_bw()
      ...
      p <- ggplot(df, aes(x = x, y = y)) +
        geom_point(alpha = 0.6, na.rm = TRUE) +
        geom_smooth(method = "lm", se = FALSE, na.rm = TRUE) +
        labs(x = xlab, y = ylab, subtitle = sub_txt) +
        theme_bw()
  ...
  output$drivers_fit_table <- renderDT({
    ...
    dt <- datatable(st, options = list(dom = "t"), rownames = FALSE)
    round_cols <- intersect(c("slope", "intercept", "r2"), names(st))
    if (length(round_cols) > 0) dt <- dt %>% formatRound(columns = round_cols, digits = 4)
    if ("p_value" %in% names(st)) dt <- dt %>% formatSignif(columns = "p_value", digits = 3)
    dt
  })
  ...
  output$download_data <- downloadHandler(
    filename = function() paste0("neon_plfa_", Sys.Date(), ".csv"),
  ...
  output$download_plots <- downloadHandler(
    filename = function() paste0("neon_plfa_plots_", input$display_site, "_", Sys.Date(), ".zip"),
    content = function(file) {
      req(display_df())
      d <- display_df()
      ...
      plots <- list(
        ...
        ts_stress = ts_plot(d, "stress_index_cy19", "Stress index (cy19:0 / 18:1ω7c)"),
        ...
        season_stress = season_plot(d, "stress_index_cy19", "Stress index (cy19:0 / 18:1ω7c)")
      )

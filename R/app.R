...
ui <- fluidPage(
  ...
  sidebarLayout(
    sidebarPanel(
      ...
      passwordInput(
        "neon_token",
        "NEON API token (recommended):",
        value = Sys.getenv("NEON_TOKEN", unset = ""),
        placeholder = "Paste your NEON API token…"
      ),
      tags$p(
        style = "font-size: 0.85em; color: #666; margin-top: -8px;",
        "NEON now requires a token for data downloads. Get one free from your ",
        tags$a(href = "https://data.neonscience.org/myaccount",
               target = "_blank", rel = "noopener", "NEON account"),
        " (see the ",
        ...
        "). Leave blank to try anonymous (rate-limited) access. Pre-filled from the ",
        tags$code("NEON_TOKEN"), " environment variable when set."
      ),

# R/neon_plfa.R
# NEON PLFA Explorer - backend functions
...
fetch_url_text <- function(url) {
  ...
  if (requireNamespace("curl", quietly = TRUE)) {
    h <- curl::new_handle()
    curl::handle_setopt(h, useragent = "R NEON PLFA Explorer")
  ...
  if (requireNamespace("httr", quietly = TRUE)) {
    r <- httr::GET(url, httr::user_agent("R NEON PLFA Explorer"))
...
# -------- robust extraction of site codes from possibly nested API structures --------
...
download_neon_product <- function(dpID, site, release,
  ...
  # NEON API token: pass through only when a non-empty token is supplied.
  # loadByProduct treats a missing token as anonymous (rate-limited) access.
  if (!is.null(token) && !is.na(token) && nzchar(trimws(token))) {
    args$token <- trimws(token)
  }
...
pull_plfa_table <- function(site, release, startdate = NULL, enddate = NULL, cache_dir = "cache/neon_downloads", token = NULL) {
...
pull_soil_periodic_tables <- function(site, release, startdate = NULL, enddate = NULL, cache_dir = "cache/neon_downloads", token = NULL) {
  ...
  ph <- soil$sls_soilpH %||% tibble::tibble()

  core <- standardize_collect_datetime(core)
  moist <- standardize_collect_datetime(moist)
  chem <- standardize_collect_datetime(chem)
  ph <- standardize_collect_datetime(ph)

  list(core = core, moisture = moist, chemistry = chem, ph = ph)
}

pull_megapit_soil_type <- function(site, release, cache_dir = "cache/neon_downloads", token = NULL) {
...
merge_plfa_soil <- function(plfa, soil_tables) {
  ...
  ph <- soil_tables$ph %||% tibble::tibble()
  ...
  core_keep <- core %>%
    select(any_of(c("sampleID", "horizon", "plotID", "collectDateTime", "collectDate", "year", "month", soilTemp_col,
                    "sampleTopDepth", "sampleBottomDepth", "biomassSampleCondition"))) %>%
  ...
  out <- plfa %>%
    ...
    left_join(ph_keep, by = "sampleID", suffix = c("", ".ph"))
...
compute_soil_health_metrics <- function(df) {
  ...
  # Cyclopropyl:precursor physiological-stress index (primary: cy19:0 / 18:1 ω7c).
  # Cyclopropyl fatty acids form from their monoenoic precursors under stress
  # (stationary phase, low pH, nutrient limitation); higher ratio = greater stress.
  cyclo19_col    <- lipid_col(df, "cyclo19To0Concentration")
  # NEON stores the 18:1 ω7c precursor (cis-vaccenic) under the field name c18To1n11;
  # the c18To1n7 / cis18To1n7 names do not exist in the product, so try n11 first.
  precursor7_col <- lipid_col(df, "c18To1n11Concentration")
  if (is.na(precursor7_col)) precursor7_col <- lipid_col(df, "c18To1n7Concentration")
  if (is.na(precursor7_col)) precursor7_col <- lipid_col(df, "cis18To1n7Concentration")
  if (!is.na(cyclo19_col) && !is.na(precursor7_col)) {
    cy19 <- suppressWarnings(as.numeric(df[[cyclo19_col]]))
    pre7 <- suppressWarnings(as.numeric(df[[precursor7_col]]))
    df$stress_index_cy19 <- ifelse(is.na(cy19) | is.na(pre7) | pre7 <= 0, NA_real_, cy19 / pre7)
  } else {
    df$stress_index_cy19 <- NA_real_
  }
...
run_neon_plfa <- function(sites, release,
                          start_date = NULL, end_date = NULL,
                          cache_dir = "cache/neon_downloads",
                          token = NULL) {

  # A NEON API token is required. Callers must supply their own token;
  # there is no anonymous or environment-variable fallback.
  if (is.null(token) || is.na(token) || !nzchar(trimws(token))) {
    stop("A NEON API token is required. Get one free from your NEON account ",
         "(https://data.neonscience.org/myaccount) and pass it via the 'token' argument.",
         call. = FALSE)
  }
  ...
  for (s in sites) {
    plfa <- pull_plfa_table(s, release, startdate = start_date, enddate = end_date, cache_dir = cache_dir, token = token)
    soil <- pull_soil_periodic_tables(s, release, startdate = start_date, enddate = end_date, cache_dir = cache_dir, token = token)

    merged <- merge_plfa_soil(plfa, soil)

    st <- pull_megapit_soil_type(s, release, cache_dir = cache_dir, token = token)

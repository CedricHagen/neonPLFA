...
compute_plfa_indicators <- function(core_microb_df) {
  ...
  # NEON field names: anteiso is aC15To0 / c17To0Anteiso; 10-methyl is lipid10Methyl*.
  gram_pos <- c(
    "i14To0ScaledConcentration",
    "i15To0ScaledConcentration", "aC15To0ScaledConcentration",
    "i16To0ScaledConcentration",
    "i17To0ScaledConcentration", "c17To0AnteisoScaledConcentration",
    "lipid10Methyl16To0ScaledConcentration", "lipid10Methyl17To0ScaledConcentration", "lipid10Methyl18To0ScaledConcentration"
  )
  # NEON stores 18:1 ω7c (cis-vaccenic) under the field name c18To1n11; c18To1n7 does not exist.
  gram_neg <- c(
    "c16To1n7ScaledConcentration", "cis16To1n7ScaledConcentration",
    "c18To1n11ScaledConcentration", "c18To1n7ScaledConcentration", "cis18To1n7ScaledConcentration",
  ...
  # Cyclopropyl fatty acids are synthesized from their monoenoic precursors under
  # physiological stress (stationary phase, low pH, nutrient limitation), so a
  # higher ratio indicates greater stress. Robust to NEON column-name variants.
  cyclo19    <- row_sum_any_of(df, c("cyclo19To0ScaledConcentration"))
  # 18:1 ω7c precursor is stored under c18To1n11 in NEON; c18To1n7 / cis18To1n7 do not exist.
  precursor7 <- row_sum_any_of(df, c("c18To1n11ScaledConcentration", "c18To1n7ScaledConcentration", "cis18To1n7ScaledConcentration"))
  stress <- cyclo19 / precursor7
  stress[!is.finite(stress)] <- NA_real_

  df$fungal_plfa_nmol_g <- fungal_plfa
  df$bacterial_plfa_nmol_g <- bacterial_plfa
  df$fb_ratio <- fb
  df$stress_index_cy19 <- stress

  df
}

# --- Main wrapper: download + merge + compute ----------------
run_neon_plfa_batch <- function(
    sites,
    start_date,
    end_date,
    release = "current",

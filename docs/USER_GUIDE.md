...
- **Cyclopropyl:precursor stress index** (cy19:0/18:1ω7c; a physiological-stress indicator)
...
### E) Soil Organic C, N, Isotopes, and CN Ratio
...
- Exact PLFA marker columns used for fungal and bacterial metrics
...
NEON now requires an API token for data downloads. Paste your token into the **NEON API token** field in the app sidebar before clicking **Run** (get one free from your [NEON account](https://data.neonscience.org/myaccount); see the [token tutorial](https://www.neonscience.org/resources/learning-hub/tutorials/neon-api-tokens-tutorial)). Alternatively, set a `NEON_TOKEN` environment variable and the field will be pre-filled automatically. Leaving it blank falls back to anonymous, rate-limited access.
...
- Cite relevant literature supporting PLFA interpretation where appropriate
